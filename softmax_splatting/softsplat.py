...
kernel_Maxsplat_updateOutput = '''
	extern "C" __global__ void kernel_Softsplat_updateOutput(
		const int n,
		const float* input,
		const float* flow,
		float* output
	) { for (int intIndex = (blockIdx.x * blockDim.x) + threadIdx.x; intIndex < n; intIndex += gridDim.x) {
		...
		& (VALUE_4(input, intN, 3, intY, intX) >= output[OFFSET_4(output, intN, 3, intNorthwestY, intNorthwestX)])
		) {
			atomicExch(&output[OFFSET_4(output, intN, intC, intNorthwestY, intNorthwestX)], VALUE_4(input, intN, intC, intY, intX));
			atomicExch(&output[OFFSET_4(output, intN, 3, intNorthwestY, intNorthwestX)], VALUE_4(input, intN, 3, intY, intX));
		}

		if ((intNortheastX >= 0) & (intNortheastX < SIZE_3(output)) & (intNortheastY >= 0) & (intNortheastY < SIZE_2(output))
		& (VALUE_4(input, intN, 3, intY, intX) >= output[OFFSET_4(output, intN, 3, intNortheastY, intNortheastX)])
		) {
			atomicExch(&output[OFFSET_4(output, intN, intC, intNortheastY, intNortheastX)], VALUE_4(input, intN, intC, intY, intX));
			atomicExch(&output[OFFSET_4(output, intN, 3, intNortheastY, intNortheastX)], VALUE_4(input, intN, 3, intY, intX));
		}

		if ((intSouthwestX >= 0) & (intSouthwestX < SIZE_3(output)) & (intSouthwestY >= 0) & (intSouthwestY < SIZE_2(output))
		& (VALUE_4(input, intN, 3, intY, intX) >= output[OFFSET_4(output, intN, 3, intSouthwestY, intSouthwestX)])
		){
			atomicExch(&output[OFFSET_4(output, intN, intC, intSouthwestY, intSouthwestX)], VALUE_4(input, intN, intC, intY, intX));
			atomicExch(&output[OFFSET_4(output, intN, 3, intSouthwestY, intSouthwestX)], VALUE_4(input, intN, 3, intY, intX));
		}

		if ((intSoutheastX >= 0) & (intSoutheastX < SIZE_3(output)) & (intSoutheastY >= 0) & (intSoutheastY < SIZE_2(output))
		& (VALUE_4(input, intN, 3, intY, intX) >= output[OFFSET_4(output, intN, 3, intSoutheastY, intSoutheastX)])
		){
			atomicExch(&output[OFFSET_4(output, intN, intC, intSoutheastY, intSoutheastX)], VALUE_4(input, intN, intC, intY, intX));
			atomicExch(&output[OFFSET_4(output, intN, 3, intSoutheastY, intSoutheastX)], VALUE_4(input, intN, 3, intY, intX));
		}
	} }
'''
...
class _FunctionSoftsplat(torch.autograd.Function):
	@staticmethod
	def forward(self, input, flow, strType):
		...
		if input.is_cuda == True:
			...
			if strType != 'max':
				...
				cupy_launch('kernel_Softsplat_updateOutput', cupy_kernel('kernel_Maxsplat_updateOutput', {
					'input': input,
					'flow': flow,
					'output': output
				}))(
...
def FunctionSoftsplat(tenInput, tenFlow, tenMetric, strType):
	...
	assert(strType in ['summation', 'average', 'linear', 'softmax', 'gumbel_softmax', 'max'])
	...
	elif strType == 'gumbel_softmax':
		tenMetric = gumbel_softmax_sample(tenMetric, 0.1)
		tenInput = torch.cat([tenInput * tenMetric.exp(), tenMetric.exp()], 1)

	elif strType == 'max':
		tenInput = torch.cat([tenInput, tenMetric], 1)

	# end

	tenOutput = _FunctionSoftsplat.apply(tenInput, tenFlow, strType)

	if strType != 'summation' and strType != 'max':
...
def gumbel_softmax_sample(logits, temperature):
	...
	sample_gumbel = -Variable(torch.log(-torch.log(U + eps) + eps))
